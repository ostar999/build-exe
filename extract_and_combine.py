...
name_replace = {
    ...
    "医院感染新发例次数": "新发感染例次数",
    "确定时段或时点医院感染患者数": "感染人数",
    "确定时段或时点医院感染例次数": "感染例次数",
    "应当报告而未报告的医院感染病例数": "漏报病例数",
    "同期应报告医院感染病例总数": "新发感染人数",
    "受调查的医务人员实际实施手卫生次数": "实际实施手卫生次数",
    "同期调查中应实施手卫生次数": "应实施手卫生次数",
    "发生I类切口手术部位感染病例数": "Ⅰ类手术部位感染例次数",
    "同期接受I类切口手术患者总数": "Ⅰ类手术例数",
    "血管内导管相关血流感染例次数": "血管导管相关血流感染例次数",
    "同期患者使用血管内导管留置总天数": "中心静脉插管使用天数",
    "呼吸机相关肺炎例次数": "呼吸机相关肺炎感染例次数",
    "导尿管相关泌尿系感染例次数": "导尿管相关泌尿道感染例次数",
    "同期患者使用导尿管总天数": "导尿管使用天数"
}

# 汇总表中指标的显示顺序
RESULT_ROW_ORDER = [
    "医院感染新发病例数", "医院感染新发例次数", "确定时段或时点医院感染患者数",
    "确定时段或时点医院感染例次数", "应当报告而未报告的医院感染病例数",
    "同期应报告医院感染病例总数",
    "受调查的医务人员实际实施手卫生次数",
    "同期调查中应实施手卫生次数",
    "发生I类切口手术部位感染病例数",
...
def find_excel_files():
    ...
    for file_type, pattern in FILE_PATTERNS.items():
        # 查找匹配该模式的所有文件（不区分大小写）
        matched_files = [f for f in glob.glob('*.xlsx')
